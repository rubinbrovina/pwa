...
export class PsComponent implements OnInit {
  ...
  ngOnInit() {
    this.service.getPSActivity(this.value1, this.value2).subscribe((x) => {
  ...
  getFirstDate(e){
    let firstDate = new Date(e.value).valueOf()
     this.service.getPSActivity(firstDate, this.value2).subscribe(x=>{
  ...
  getSecondDate(e){
   ...
   this.service.getPSActivity(this.value1, secondDate).subscribe(x=>{
